...
function checkCipherStatus() {
    ...
    chrome.runtime.sendMessage({action: 'checkCipherStatus'}, function(response) {
        const cipherContainer = document.getElementById('cipher-status-container');
        if (!cipherContainer) {
            ...
            statusContainer.parentNode.insertBefore(cipherDiv, statusContainer.nextSibling);
        }
        
        const container = document.getElementById('cipher-status-container');
        if (response && response.success) {
            if (response.autoStart) {
                const initDate = response.initialized ? new Date(response.initialized).toLocaleString() : '不明';
                container.innerHTML = `
                    <div class="status working">
                        🔐 Cipher: 自動起動有効
...
function setupEventListeners() {
    ...
    document.getElementById('test-setup-guide').addEventListener('click', function() {
        chrome.runtime.getBackgroundPage((backgroundPage) => {
            if (backgroundPage && backgroundPage.testFunctions) {
    ...
    document.getElementById('test-cipher-init')?.addEventListener('click', function() {
        chrome.runtime.sendMessage({action: 'initializeCipher'}, function(response) {
            if (response && response.success) {
    ...
    document.getElementById('test-cipher-status')?.addEventListener('click', function() {
        checkCipherStatus();
    });
}
